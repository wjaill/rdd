    ...
    export default {
        ...
        data(){
            ...
            methods:{
                ...
                axios.post('/api/v1/user/changepd/',{uid:that.u_id,old_password:that.oldpassword,new_password:that.newpassword}).then((res)=>{
                    console.log(res)
                    if(res.data.code==1) {
                        Toast('原密码输入错误')
                        return false;
        ...
        mounted(){  
            ...
            that.u_id = that.data.LoginOnData.uid
        },
    }    
</script>
